#!/bin/bash
# This script will download the lastest Yuzu EA AppImage in the 'image' sub directory
# and will create a Symlink to it for ease of switching between Main and EA

# Set GitHub release URL
export V_GH_URL="https://api.github.com/repos/pineappleEA/pineapple-src/releases/latest"
# Set directory to save the AppImage to
export V_DIR=image
# Set file name of download
export V_DL_OUTPUT="Linux-Yuzu-EA.AppImage"
# Symlink Name
...
# If a Yuzu EA AppImage already exists...
...
V_DL_URL=$(curl --silent $V_GH_URL | grep -Po '"browser_download_url": "\K.*\.AppImage')
